                    ...
                    features_cache.append(
                        {
                            ...
                            "embedding": embedding,
                            "attributes": {"color": color, "type": type_str},
                        }
                    )
    ...
    for event_data in sorted_events:
        ...
        event = TrackEvent(
            ...
            attributes=Attributes(
                color=event_data["attributes"]["color"], type=event_data["attributes"]["type"]
            ),
        )
...
def load_ground_truth(videos_map, num_frames):
    ...
    for cid, path in videos_map.items():
        ...
        for _, row in df_gt.iterrows():
            ...
            gt_records.append([cid, track_id, frame_id, x, y, w, h, -1.0, -1.0])
    ...
    print(
        f"  Loaded {len(df_gt_all)} ground truth records across cameras: {list(videos_map.keys())}"
    )
    return df_gt_all
...
def main():
    ...
    try:
        summary_baseline = aicity_eval(
            df_gt.copy(), df_pred_baseline.copy(), roidir="dataset/test/S06", dstype=""
        )
        aicity_print_results(summary_baseline)
    except Exception as e:
        print(f"Error evaluating baseline: {e}")
        print(
            "Note: Baseline tracking has no cross-camera matches, so all baseline trajectories are filtered out by the evaluator."
        )

    print("\n--- DMT REID ENABLED ---")
    try:
        summary_reid = aicity_eval(
            df_gt.copy(), df_pred_reid.copy(), roidir="dataset/test/S06", dstype=""
        )

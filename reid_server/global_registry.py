...
class GlobalRegistry:
    ...
    def add_identity(
        self, embedding: np.ndarray, cls_label: str, attributes: dict, timestamp: float
    ) -> int:
        global_id = self._next_id
        self._next_id += 1
        self.identities[global_id] = GlobalIdentity(
            global_id, embedding, cls_label, attributes, timestamp
        )
        return global_id
    ...
    def update_identity(
        self, global_id: int, embedding: np.ndarray, attributes: dict, timestamp: float
    ):

...
class Bottleneck_IBN(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, baseWidth, cardinality, stride=1, downsample=None, ibn=None
    ):
        """Constructor
        Args:
...
class ResNeXt_IBN(nn.Module):
    def __init__(
        self, baseWidth, cardinality, layers, ibn_cfg=("a", "a", "a", None), num_classes=1000
    ):
    ...
    def _make_layer(self, block, planes, blocks, stride=1, ibn=None):
        ...
        layers = []
        layers.append(
            block(self.inplanes, planes, self.baseWidth, self.cardinality, stride, downsample, ibn)
        )
        self.inplanes = planes * block.expansion
        for i in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, self.baseWidth, self.cardinality, 1, None, ibn)
            )

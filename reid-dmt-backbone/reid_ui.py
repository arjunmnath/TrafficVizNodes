...
class RichUIListener(ReIDPipelineListener):
    ...
    def show_configuration(self, config_data: dict):
        config_table = Table(box=box.SIMPLE_HEAVY, show_header=True, header_style="bold magenta", expand=False)
        ...
        console.print(Panel(config_table, title="[bold magenta]ReID Test Pipeline Configuration[/bold magenta]", expand=False))

    def on_init_start(self):
        self.status = console.status("[bold yellow]Initializing ReID pipeline and models...", spinner="dots")
        self.status.start()
    ...
    def on_video_end(self, video_path: str, total_frames: int):
        ...
        self.recent_logs.append(f"Completed processing {video_name}: {total_frames} frames analyzed.")
    ...
    def on_pipeline_end(self, registry: SimpleRegistry, output_path: str):
        ...
        console.print("\n")
        console.print(Panel(
            Text("DMT RE-IDENTIFICATION FINAL MATCHING REPORT", style="bold white on blue", justify="center"),
            border_style="blue",
            box=box.DOUBLE
        ))
        ...
        for item in summary:
            ...
            source_info = ", ".join([f"[bold cyan]{v}[/bold cyan]: {c} frames" for v, c in vid_counts.items()])
    ...
    def make_layout(self, video_name, video_idx, num_videos, frame_count, total_frames, elapsed_time, fps, registry):
        layout = Layout()
        layout.split_column(
        ...
        if total_frames > 0:
            ...
            metrics_lines.append(f"    Frame:      [white]{frame_count}/{total_frames}[/white] ([cyan]{pct:.1f}%[/cyan])")
        else:
        ...
        metrics_lines.append(f"    Total Unique Identities: [bold green]{len(registry.identities)}[/bold green]")
        ...
        layout["left"].update(Panel(metrics_text, title="[bold cyan]System Metrics[/bold cyan]", border_style="cyan", box=box.ROUNDED))
        ...
        layout["footer"].update(Panel(log_text, title=logs_title, border_style="blue", box=box.ROUNDED))
...
class HeadlessUIListener(ReIDPipelineListener):
    ...
    def on_video_start(self, video_path: str, video_idx: int, total_videos: int, total_frames: int, fps: float):
        video_name = os.path.basename(video_path)
        print(f"[{time.strftime('%H:%M:%S')}] [VIDEO START] Processing video {video_idx}/{total_videos}: {video_name} (Total frames: {total_frames}, FPS: {fps:.1f})")
    ...
    def on_frame_processed(self, video_name: str, video_idx: int, total_videos: int, frame_count: int,
        ...
        if frame_count > 0 and frame_count % 100 == 0:
            ...
            print(f"[{time.strftime('%H:%M:%S')}] [PROGRESS] Video {video_idx}/{total_videos} | Frame {frame_count}/{total_frames} ({pct:.1f}%) | Speed: {fps:.1f} FPS | Unique Identities: {len(registry.identities)}")

    def on_video_end(self, video_path: str, total_frames: int):
        video_name = os.path.basename(video_path)
        print(f"[{time.strftime('%H:%M:%S')}] [VIDEO END] Completed processing {video_name}: {total_frames} frames analyzed.\n")
